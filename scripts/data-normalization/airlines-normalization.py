...
INPUT_PATH = RAW_PATH / "airlines.csv"
OUTPUT_PATH = RAW_PATH / "airlines_normalized.csv"
...
def validate_keys(df: pd.DataFrame) -> None:
    if "IATA_CODE" not in df.columns:
        log("WARN", "VALIDATE", "Coluna IATA_CODE não encontrada no dataset de airlines")
        return
...
def normalize_airlines() -> pd.DataFrame:
...
def main():
    log("INFO", "PIPELINE", "Iniciando pipeline de normalização de airlines")

    try:
        df = normalize_airlines()
